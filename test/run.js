...
function attachLogger(node, label) {
  node.on("request", ({ identityText, payload }) => {
    ...
    console.log(`    [${label}] ◀ REQ${from}  "${fmt(payload)}"`);
  });

  node.on("response", ({ identityText, payload }) => {
    if (logSilent) return;
    const from = identityText ? `  from=${identityText}` : "";
    console.log(`    [${label}] ◀ RES${from}  "${fmt(payload)}"`);
  });
}
...
const AUTH_KEY = "test-secret-key";
...
await test("并发 100 条请求（全部内容一致性校验）", async () => {
  ...
  logSilent = false;
  console.log(`    [并发] 全部完成：发出 100 条 / 收到 ${results.length} 条`);
...
await test("错误 authKey → master 丢弃请求并触发 auth_failed 事件", async () => {
  ...
  master.once("auth_failed", ({ identityText, authKey }) => {
    authFailedFired = true;
    console.log(
      `    [MASTER] ✗ AUTH  from=${identityText}  provided="${authKey ?? "<none>"}"`,
    );
  });
